...
import { ShieldCheck, ChevronDown, Unplug, Wallet, FileText, Search, Clock, AlertTriangle } from 'lucide-react'
...
export default function Home() {
  const { activeWallet, isConnected, isWrongNetwork, connect, disconnect, switchToExpectedChain } = useWallet()
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/40 dark:from-zinc-950 dark:via-zinc-900 dark:to-indigo-950/30">
      ...
      <header className="sticky top-0 z-30 border-b border-zinc-200/80 dark:border-zinc-800/80 bg-white/75 dark:bg-zinc-900/75 backdrop-blur-md">
              ...
              <button
                ...
                className={`flex items-center gap-2 px-3 py-2 rounded-xl border text-sm font-medium transition-colors ${
                  isWrongNetwork
                    ...
                    : 'border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 text-zinc-700 dark:text-zinc-300 hover:border-indigo-300 dark:hover:border-indigo-700'
                }`}
              >
                <span className={`w-2 h-2 rounded-full shrink-0 ${isWrongNetwork ? 'bg-amber-500' : 'bg-emerald-500 animate-pulse'}`} />
                <span className="font-mono">{shortAddress(activeWallet!.address)}</span>
                <ChevronDown className="w-3.5 h-3.5 text-zinc-400 shrink-0" />
              </button>
            )}

            {/* Dropdown de estado de wallet */}
            {walletOpen && isConnected && (
              <div className="absolute right-0 mt-2 w-72 rounded-2xl border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-xl shadow-zinc-900/10 dark:shadow-zinc-900/50 z-50 overflow-hidden">
                <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 flex items-center gap-2">
                  ...
                  <p className="text-xs font-mono text-zinc-600 dark:text-zinc-300 break-all">{activeWallet!.address}</p>
                </div>
                ...
                {isWrongNetwork && (
                  <div className="px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 space-y-2">
                    ...
                    <button
                      ...
                      className="w-full px-3 py-1.5 rounded-lg bg-amber-500 hover:bg-amber-600 text-white text-xs font-medium transition-colors"
                    >
                      Switch network
                    </button>
                  </div>
        ...
        {connectError && (
          <div className="max-w-4xl mx-auto px-4 sm:px-6 pb-3 -mt-1">
      ...
      <footer className="py-6 text-center text-xs text-zinc-400 dark:text-zinc-600">
        Autenticidad de Documentos Digitales &middot; Ethereum
      </footer>
    </div>
  )
}
